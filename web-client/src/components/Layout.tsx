    ...
    <div className="min-h-screen bg-gray-50 dark:bg-dark-bg">
      <aside className={`sidebar ${sidebarOpen ? 'translate-x-0' : ''}`}>
        <div className="flex h-full flex-col">
              ...
              {filteredMenuItems.map((item) => {
                ...
                return (
                  <Link
                    key={item.href}
                    href={item.href}
                    className={`flex items-center px-3 py-1.5 text-[13px] font-medium rounded-sm transition-colors ${
                      isActive
          ...
          <div className="p-3 border-t border-gray-200 dark:border-white/[0.08] space-y-1.5">
            <button
              onClick={toggleTheme}
              className="flex items-center w-full px-3 py-1.5 text-[13px] font-medium text-gray-700 hover:text-gray-900 hover:bg-gray-100 dark:text-white/60 dark:hover:text-white/80 dark:hover:bg-white/5 rounded-sm transition-colors"
      ...
      <main className="main-content">
        <div className="px-6 pb-6 lg:px-6 lg:pb-6 pt-5 lg:pt-6">
          <button
            onClick={() => setSidebarOpen(true)}
            className={`lg:hidden fixed top-4 left-4 z-50 p-2 rounded-sm bg-white dark:bg-dark-surface text-gray-400 hover:text-gray-500 hover:bg-gray-100 dark:hover:bg-white/5 shadow-medium transition-opacity duration-200 ${sidebarOpen ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}
          >
