...
AUTH_EXPIRE_TIME=30                 # Access token lifetime value
AUTH_EXPIRE_TIME_FREQ=minutes       # seconds|minutes|hours|days|weeks
AUTH_REFRESH_EXPIRE_TIME=7          # Refresh token lifetime value
AUTH_REFRESH_EXPIRE_FREQ=days       # seconds|minutes|hours|days|weeks

# Secrets & Encryption
# TOKEN_ENCRYPTION_KEY encrypts API keys at rest (token defs, user API keys)
# MEM_ENCRYPTION_KEY encrypts in-memory DB dumps; also used as fallback for token encryption if TOKEN_ENCRYPTION_KEY is unset
...
- AUTH_EXPIRE_TIME: numeric value for access token lifetime. Default 30.
- AUTH_EXPIRE_TIME_FREQ: unit for access token lifetime. Default minutes.
- AUTH_REFRESH_EXPIRE_TIME: numeric value for refresh token lifetime. Default 7.
- AUTH_REFRESH_EXPIRE_FREQ: unit for refresh token lifetime. Default days.
- Supported units: seconds, minutes, hours, days, weeks (aliases like s/sec, m/min, h/hr, d, w are accepted).
...
AUTH_EXPIRE_TIME=2
AUTH_EXPIRE_TIME_FREQ=hours

# 14-day refresh tokens
AUTH_REFRESH_EXPIRE_TIME=14
