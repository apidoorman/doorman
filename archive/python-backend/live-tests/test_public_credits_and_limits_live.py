...
def _ok_status(code: int, text: str = "") -> bool:
    """Check if status is acceptable (not auth failure, tolerates upstream issues)."""
    if "edgesuite.net" in text or "Access Denied" in text or "w3schools" in text:
        return True
...
def _exercise_credits(client: LiveClient, kind: str, name: str, ver: str, meta: Dict[str, Any]):
    ...
    for _ in range(5):
        ...
        assert _ok_status(r.status_code, r.text), r.text
...
def _exercise_concurrent_credits(
    ...
    ok = sum(1 for r in results if _ok_status(r.status_code, r.text))
...
def test_live_api_credits_limits_and_cleanup(client: LiveClient, kind: str, idx: int):
    ...
    if r0.status_code == 500:
        try:
            ...
            pass
    assert _ok_status(r0.status_code, r0.text), r0.text
    ...
    try:
        ...
        assert _ok_status(r1.status_code, r1.text), r1.text
    ...
    if not (_test_mode or _skip_tier):
        ...
        try:
            ...
            assert _ok_status(r3.status_code, r3.text), r3.text
