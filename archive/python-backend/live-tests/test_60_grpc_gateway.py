...
def test_grpc_gateway_basic_flow(client):
    ...
    try:
        ...
        # Use api_name as proto filename so the global protobuf descriptor pool
        # never sees a duplicate file name across test runs in the same process.
        proto_stem = api_name
        with tempfile.TemporaryDirectory() as td:
            tmp = pathlib.Path(td)
            (tmp / f'{proto_stem}.proto').write_text(proto)
            out = tmp / 'gen'
            out.mkdir()
            code = protoc.main(
                [
            ...
            pb2 = importlib.import_module(f'{proto_stem}_pb2')
            pb2_grpc = importlib.import_module(f'{proto_stem}_pb2_grpc')

            from concurrent import futures

            import grpc

            ServicerBase = getattr(pb2_grpc, 'GreeterServicer', getattr(pb2_grpc, 'Greeter', object))

            class Greeter(ServicerBase):
                def Hello(self, request, context):
                    return pb2.HelloReply(message=f'Hello, {request.name}!')
                def __getattr__(self, name):
                    return lambda req, ctx: pb2.HelloReply(message="ok")

            server = grpc.server(futures.ThreadPoolExecutor(max_workers=2))
            add_func = getattr(pb2_grpc, 'add_GreeterServicer_to_server', getattr(pb2_grpc, 'add_ResourceServicer_to_server', None))
            add_func(Greeter(), server)
            import socket
