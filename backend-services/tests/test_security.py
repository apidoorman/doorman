...
async def test_jwt_tamper_rejected(client):
    # Craft a JWT with the wrong secret; should be rejected
...
async def test_csrf_required_when_https(monkeypatch, authed_client):
    # Force HTTPS-enabled, do not send CSRF header -> 401
...
async def test_header_injection_is_sanitized(monkeypatch, authed_client):
    # Onboard API that allows a custom header
    ...
    # Patch client to capture headers forwarded downstream
    ...
    # Avoid rate/throttle and group checks to focus on header path
    ...
    # No CRLF or HTML tags should survive sanitization
...
async def test_rate_limit_enforced(monkeypatch, authed_client):
    # Reduce admin rate to 1 per second
    ...
    # Purge user cache to pick up new limits
    ...
    # Avoid group/subscription checks to focus on limiter
    import routes.gateway_routes as gr
    async def _pass_sub(req): return {"sub": "admin"}
    async def _pass_group(req, full_path: str = None, user_to_subscribe=None): return {"sub": "admin"}
    monkeypatch.setattr(gr, "subscription_required", _pass_sub)
    ...
    # Second request should be rate limited (429) given limit=1
