...
async def test_security_and_memory_dump_restore(authed_client):
    # Grant manage_security to admin role
...
async def test_onboard_public_apis_for_all_gateway_types(monkeypatch, authed_client):
    # Onboard 2 REST APIs with endpoints
    rest_apis = [
    ...
    for name, ver, servers, endpoints in rest_apis:
        ...
        # Subscribe admin to use gateway
    ...
    # Onboard 2 SOAP APIs (no endpoint docs required for gateway path)
    soap_apis = [
    ...
    # Onboard 2 gRPC APIs (placeholder servers)
    grpc_apis = [
    ...
    # Sanity: call REST for one of the newly onboarded with mocked client
    ...
    # Avoid Redis limit/throttle and strict group/subscription checks in router layer
