...
async def test_gateway_rest_happy_path(monkeypatch, authed_client):
    # Create API and endpoint and subscribe
    api_payload = {
        "api_name": "echo",
        "api_version": "v1",
    ...
    # Monkeypatch httpx.AsyncClient used by gateway to avoid network
    import services.gateway_service as gs
    # Patch where used in the router module
...
async def test_gateway_graphql_grpc_soap(monkeypatch, authed_client):
    # Prepare minimal APIs and subscriptions for each
    ...
    # Missing header case for GraphQL
    missing = await authed_client.post("/api/graphql/graph", json={"query": "{ping}"})
    assert missing.status_code == 400

    # Monkeypatch service methods to short-circuit network
    import services.gateway_service as gs
    import routes.gateway_routes as gr
    ...
    # Also bypass subscription/group checks to isolate gateway path
