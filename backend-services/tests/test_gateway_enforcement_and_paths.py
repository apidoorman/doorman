...
async def test_subscription_required_blocks_without_subscription(monkeypatch, authed_client):
    # Create API and endpoint but do not subscribe
    name, ver = "nosub", "v1"
...
async def test_group_required_blocks_when_disallowed_group(monkeypatch, authed_client):
    # Create API that only allows a group the admin user does not belong to
    name, ver = "nogroup", "v1"
    await authed_client.post(
        "/platform/api",
        json={
    ...
    # Bypass subscription check to isolate group enforcement error
...
async def test_response_header_filtering_excludes_unlisted(monkeypatch, authed_client):
    # Configure API to allow only "content-type"; X-Upstream should not be returned
    ...
    assert r.status_code == 200
    # Middleware always adds content-type; ensure X-Upstream is not echoed back
    assert r.headers.get("X-Upstream") is None


@pytest.mark.asyncio
async def test_authorization_field_swap(monkeypatch, authed_client):
    # Create API with auth swap; gateway should map x-token -> Authorization for upstream call
    ...
    assert r.status_code == 200
    # Upstream echo should show Authorization header set
