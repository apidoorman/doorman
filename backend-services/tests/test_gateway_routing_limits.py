...
class _FakeHTTPResponse:
    def __init__(self, status_code=200, json_body=None, text_body=None, headers=None):
        ...
        # Simulate upstream returning a custom header along with content-type
...
async def test_routing_precedence_and_round_robin(monkeypatch, authed_client):
    ...
    # Add endpoint with endpoint servers
    ...
    # Use fake HTTP client to avoid network
    import services.gateway_service as gs
    monkeypatch.setattr(gs.httpx, "AsyncClient", _FakeAsyncClient)

    # With client-key header, should use routing servers first
    ...
    # Without client-key, endpoint servers should be used (round-robin across 2 calls)
...
async def test_client_routing_round_robin(monkeypatch, authed_client):
    ...
    await subscribe_self(authed_client, name, ver)
    # Add client routing with two servers
...
async def test_api_level_round_robin_when_no_endpoint_servers(monkeypatch, authed_client):
    ...
    # Endpoint without endpoint_servers
...
async def test_throttle_queue_limit_exceeded_returns_429(monkeypatch, authed_client):
    ...
    # Set throttle_queue_limit to 1; second request in same window should exceed
    from utils.database import user_collection
...
async def test_query_params_and_headers_forwarding(monkeypatch, authed_client):
    ...
    # Ensure limits are generous so this test isn't affected by prior limit tests
    from utils.database import user_collection
    ...
    # Response headers should include X-Upstream because it's in allowed list
...
async def test_credit_header_injection_and_user_override(monkeypatch, authed_client):
    # 1) Define a credit group with default API key/header
    ...
    # 2) Create an API that uses that credit group and subscribe admin
    ...
    # 3) Override admin's API key for that group
    ...
    # 4) Monkeypatch HTTP client and call gateway; injected header should be visible in upstream echo
    import services.gateway_service as gs
    monkeypatch.setattr(gs.httpx, "AsyncClient", _FakeAsyncClient)
...
async def test_endpoint_not_found_returns_404_code_gtw003(monkeypatch, authed_client):
    from conftest import create_api, subscribe_self  # type: ignore
    name, ver = "noep", "v1"
    await create_api(authed_client, name, ver)
    await subscribe_self(authed_client, name, ver)
