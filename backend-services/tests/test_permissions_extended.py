...
async def test_non_admin_role_cannot_access_monitor_or_credits(monkeypatch, authed_client):
    # Create a limited role and user
    ...
    # Monitor metrics require manage_gateway
    mm = await viewer.get("/platform/monitor/metrics")
    assert mm.status_code == 403

    # Credits creation requires manage_credits
    ...
    # Clearing caches requires manage_gateway
...
async def test_endpoint_validation_management_requires_permission(authed_client):
    # Create a role lacking manage_endpoints and a user with it
    ...
    # Create an endpoint as admin to target
    ...
    # Attempt to create endpoint validation should be forbidden
