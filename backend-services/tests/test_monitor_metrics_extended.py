...
async def test_metrics_increment_on_gateway_requests(monkeypatch, authed_client):
    ...
    # Create two small APIs and endpoints
    ...
    # Monkeypatch upstream to avoid network
    ...
    # Make requests that should hit /api/* paths and be counted
...
async def test_metrics_top_apis_aggregate(monkeypatch, authed_client):
    ...
    # top_apis is list of [api, count]; ensure at least one rest:* entry exists
...
async def test_monitor_report_csv(monkeypatch, authed_client):
    # Make at least one API request to ensure metrics fallback can fill report
    from conftest import create_api, create_endpoint, subscribe_self  # type: ignore
    name, ver = "mapi4", "v1"
    await create_api(authed_client, name, ver)
    ...
    # Narrow date range around now to use metrics fallback
    ...
    # CSV should contain headings like Report, Overview, Status Codes, API Usage
    text = csvr.text
