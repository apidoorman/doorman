...
async def available_apis(username: str, request: Request):
    ...
    try:
        ...
        logger.info(f"{request_id} | Endpoint: {request.method} {str(request.url.path)}")
        # Determine if actor has manage_subscriptions via JWT accesses or role lookup
        ...
        # Non-admin: only allow APIs intersecting with actor's groups; block listing others' availability
        if username != actor:
            return respond_rest(ResponseModel(
                status_code=403,
                error_code="SUB009",
